...
select 시도, 이름 from 학생 order by 시도 asc, 이름 desc;
...
-- 23 <과목> 테이블에서 과목의 영문 이름이 'C'로 시작하여 'e'로 끝나는 과목의 '과목번호', '과목명', '영문명'을 가져오는 SQL문을 작성하시오.
select * from 과목;
...
-- 30 <과목> 테이블에서 '영문명'이 'T'로 시작하여 'n'으로 끝나는 과목의 '과목번호, ’과목명', '영문명'을 가져오는 SQL문을 작성하시오.
select * from 과목;
...
select concat(이름 , " " ,"(", 주소," " , 시군구," " ,시도 ," ", 우편번호, ")") as '학생정보' from 학생 order by 이름;
...
where 학과명 not in (select 학과명 from 학과 where 학과번호 = 01);

select * from 수강신청;
select 수강신청번호, 수강신청.학번, 날짜 
from 수강신청, 학생, 학과 
where 학생.학번 = 수강신청.학번  and 학생.학과 = 학과.학과번호 and 학과.학과번호 != 01;
...
select * from 수강신청내역;
select 수강신청번호, 과목번호, 평점, if(평점 = 0, '미취득', '취득') as 취득여부 from 수강신청내역 where 평점 > -1 && 평점 is not null;
...
select * from 과목;
select 과목명, 학점, sum(15*시수) as '총시간수' from 과목 group by 과목명 order by 과목명;

-- 44 <학생> 테이블에서 '학번'과 이름에서 성씨(last name)를 뺀 이름(first name)을 출력하는 SQL문을 작성하시오.
select * from 학생;
select 학번, mid(이름,2) as 이름 from 학생;
...
if(학과.학과명 <> '컴퓨터정보학과', '타과', '컴퓨터정보학과') as '비고' 
from 학생, 학과
where 학생.학과=학과.학과번호;
...
select * from 교수;
select count(distinct(사번)) as '교수 수' from 교수 where 전화번호 is not null;
...
select count(과목번호) as '신청과목 수', avg(평점) as '평균 평점' from 수강신청내역 inner join 수강신청 on 수강신청내역.수강신청번호 = 수강신청.수강신청번호 where 수강신청.수강신청번호 = 1810001;

-- 57 <수강신청내역> 테이블에서 아래와 같이 과목별로 수강자 수를 읽어오는(과목번호, 수강자 수) SQL문을 작성하시오.
select * from 수강신청내역;
select 과목번호, count(과목번호) as '수강자 수' from 수강신청내역  group by 과목번호;

-- 58 위의 문제에서 '평점'이 - 1 인 과목은 제외하고 출력하도록 필터링하시오.
select 과목번호, count(과목번호) as '수강자 수' from 수강신청내역 where not 평점 = -1 group by 과목번호;

-- 59 위의 문제에서 과목별 평균 평점을 출력하시오.
select 과목번호, count(과목번호) as '수강자 수', avg(평점) as '평균 평점' from 수강신청내역 where not 평점 = -1 group by 과목번호;

-- 60 위의 문제에서 수강자 수가 4 명 이상인 과목만을 출력하도록 필터링하시오.
select 과목번호, count(과목번호) as '수강자 수' , avg(평점) as '평균 평점' from 수강신청내역 where not 평점 = -1 group by 과목번호 having count(과목번호) >=4;

-- 61 위의 문제에서 평균평점을 기준으로 오름차순으로 정렬하시오.
select 과목번호, count(과목번호) as '수강자 수' , avg(평점) as '평균 평점' from 수강신청내역 where not 평점 = -1 group by 과목번호 having count(과목번호) >=4 order by 3;

-- 62 <수강신청내역> 테이블에서 아래와 같이 '수강신청번호별로 수강과목 수와 평균평점을 출력하시오. (단, 수강과목 수가 3 이상인 것만 출력하고 평균평점에 대하여 내림차순으로 정렬하시오).
select * from 수강신청내역;
select 수강신청번호, count(과목번호) as '수강과목 수' , avg(평점) as '평균 평점' from 수강신청내역 where not 평점 = -1 group by 수강신청번호 having count(과목번호) >=3 order by 평점 desc;

-- 63 모든 학생에 대하여 학번, 학과번호, 이름, 소속 학과 이름을 출력하는 SQL 문을 작성하시오.
select * from 학생;
select * from 학과;
select 학생.학번, 학과.학과번호, 학생.이름, (select 학과.학과명 from 학과 where 학생.학과 = 학과.학과번호 )as '학과명' from 학생, 학과 group by 학번;

-- 64 <수강신청내역> 테이블에서 수강신청번호 '1810001'와 '1610001'에 대하여 수강신청번호와 수강 과목번호, 수강 과목명 출력하는 SQL 문을 작성하시오.
 select * from 수강신청내역;
  select * from 과목;
 select 수강신청내역.수강신청번호, 수강신청내역.과목번호, 과목.과목명 from 수강신청내역, 과목 where 수강신청내역.과목번호 = 과목.과목번호 and 수강신청내역.수강신청번호 in ('1810001', '1610001');
...
select * from 수강신청;
select * from 수강신청내역;
select 수강신청.수강신청번호, 과목.과목번호, 과목명 from 수강신청내역 
inner join 과목 on 수강신청내역.과목번호 = 과목.과목번호
...
select 수강신청.수강신청번호, 수강신청내역.과목번호, 과목.과목명 from 수강신청내역, 과목, 수강신청
where 수강신청내역.수강신청번호 = 수강신청.수강신청번호 and 과목.과목번호 = 수강신청내역.과목번호 and 수강신청.학번 = '1801001';

-- 68. 학번이 '1801001'인 학생이 수강 신청한 과목 수와 평점 합계를 출력하는 SQL 문을 작성하시오.
select count(수강신청.수강신청번호) as '과목수', sum(수강신청내역.평점) as '평점합계' from 수강신청 
inner join 수강신청내역 on 수강신청.수강신청번호 = 수강신청내역.수강신청번호
inner join 학생 on 학생.학번 = 수강신청.학번
where 학생.학번 = '1801001';

-- 69 수강신청 별로 신청 학생의 학번, 이름, 평균 평점(평검합계/과목수)를 출력하는 SQL 문을 작성하시오. (단, 평점이 부여된 수강신청만 출력한다)
select 수강신청내역.수강신청번호,학생.학번, 학생.이름, avg(수강신청내역.평점) as '평균 평점' from 수강신청
inner join 수강신청내역 on 수강신청내역.수강신청번호 = 수강신청.수강신청번호
inner join 학생 on 학생.학번 = 수강신청.학번
group by 수강신청내역.수강신청번호 having avg(수강신청내역.평점) >= 0; 
...
select * from 수강신청내역;
select * from 과목;
select 수강신청번호, 과목.과목번호, 과목.과목명 from 수강신청내역
...
select * from 수강신청;
select 수강신청번호, 학생.학번, 학생.이름 from 수강신청
...
select * from 수강신청;
select * from 수강신청내역;
select 수강신청.수강신청번호, 수강신청내역.과목번호, 과목.과목명 from 수강신청내역
inner join 수강신청 on 수강신청내역.수강신청번호 = 수강신청.수강신청번호
inner join 과목 on 과목.과목번호 = 수강신청내역.과목번호
where 수강신청.학번 = '1801001';
...
select  사번, 학과, 이름 
from 교수
inner join 학과 on 교수.학과 = 학과.학과번호
where 학과 in (select 학과 from 교수 where 사번 = '1000004' );

-- 75 학생별로 학번, 이름, 수강 신청한 과목 수, 평균 평점을 출력하시오. (단, 평점이 부여된 수강 신청만 출력한다)
select * from 학생;
select * from 수강신청내역;
select * from 수강신청;
select 수강신청.학번, 학생.이름, count(수강신청.수강신청번호) as '신청 과목 수', avg(평점) as '평균 평점' from 수강신청, 학생, 수강신청내역
where 수강신청.학번 = 학생.학번 and 수강신청내역.수강신청번호 = 수강신청.수강신청번호 and 수강신청내역.평점 >=0 
group by 학생.학번;
...
select * from 수강신청내역;
select * from 수강신청;
select 수강신청내역.과목번호, 과목.과목명, count(학번) as '수강인원' , avg(평점) as '평균 평점' from 수강신청내역
inner join 수강신청 on 수강신청내역.수강신청번호 = 수강신청.수강신청번호
inner join 과목 on 과목.과목번호 = 수강신청내역.과목번호
where 수강신청.학기 = 1
group by 과목.과목명;
...
-- ---------------------------------------------------------------------------------------------------
-- 수업예제1. 과목번호가 k20002인 과목을 수강 신청한 모든 학생의 학과, 이름, 학년 조회.
select 학과, 이름, 학년 
from 학생 
...
select 학과, 이름, 학년 from 학생, 수강신청, 수강신청내역 where 학생.학번 = 수강신청.학번 and 수강신청.수강신청번호 = 수강신청내역.수강신청번호 and 과목번호 = 'K20002';

-- 수업예제2. <학생> 테이블에 있는 모든 학생들의 수강신청 횟수.
select 학번, 이름, (select count(*) from 수강신청 where 수강신청.학번 = 학생.학번 ) as 수강신청횟수 from 학생;
